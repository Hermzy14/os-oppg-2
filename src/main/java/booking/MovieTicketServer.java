...
public class MovieTicketServer {
  ...
  private volatile int availableTickets; //change this to volatile for tasks

  /**
   * Constructor
   *
   * @param movieName
   * @param availableTickets Creates a move ticket server with a given movie name and available tickets
   */
  ...
  /**
   * @param name
   * @param tickets Books tickets for a movie
   */
  public synchronized void bookTicket(String name, int tickets) {
    if (availableTickets >= tickets) {
      ...
      System.out.println(
          "Sorry, " + name + " we only have " + availableTickets + " tickets left for " +
              movieName);
    }
  }
}
